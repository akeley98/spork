...
/* Required by Sm90_mma_async_tf32(d,a,b,n=128) */
#define exo_wgmma_mma_async_m64n128k8_f32_tf32(swizzle_bits_a, swizzle_bits_b, D, smem_a, smem_b, m_matrix_stride, n_matrix_stride, k_matrix_stride) \
{ \
    auto desc_a = exo_CudaUtil::exo_matrix_descriptor<swizzle_bits_a>(smem_a, m_matrix_stride, k_matrix_stride); \
    auto desc_b = exo_CudaUtil::exo_matrix_descriptor<swizzle_bits_b>(smem_b, n_matrix_stride, k_matrix_stride); \
    asm volatile( \
...
#define exo_Sm90_store_d(ColumnMajor, RegCount, dst, src) \
{ \
      ...
      if ([[maybe_unused]] int wg = (threadIdx.x / 128); 1) {
        {
          ...
          for (int k_mma = 0; k_mma < 4; k_mma++) {
            for (int m_mma = 0; m_mma < 2; m_mma++) {
              ...
              exo_wgmma_mma_async_m64n128k8_f32_tf32(1, 1,
                    D_rmem[m_mma],
  ...
  if (int tmp = threadIdx.x; tmp < 256) {
    if ([[maybe_unused]] int wg = (threadIdx.x / 128); 1) {
      ...
      for (int m_mma = 0; m_mma < 2; m_mma++) {
        ...
        exo_Sm90_store_d(true, 64, ((struct exo_win_2f32){ &exo_deviceArgs.C[(128 * exo_task.n_task) * (exo_deviceArgs.M) + 64 * m_mma + 128 * wg + 256 * exo_task.m_task], { exo_deviceArgs.M, 1 } }), D_rmem[m_mma]);
      }
    }
  }
