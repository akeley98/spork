...
\filbreak
We will not support lowering all possible combinations of $A_1$, $A_2$, $T$ (collective lane); futhermore, we'll only support some fixed usage patterns for \lighttt{barrier} declaration, \lighttt{Arrive}, and \lighttt{Await} statements, with the intent that they'll lower in an obvious way to mbarrier or commit\_group synchronization, without surprises.
