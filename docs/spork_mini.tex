...
\filbreak
\mainKey{Invalid Instruction Divergence:} Some instructions expect to be called by a certain convergent collective unit (e.g. warp/warpgroup tensor core instructions) and we will enforce this at the call site of each instr.
...
\filbreak
The \lighttt{cuda\_tasks} loop can basically do anything an ordinary \lighttt{seq} loop is, since they're lowered to sequential loops (plus some modular arithmetic for task distribution).
...
\begin{itemize}
  \item $D \leftarrow AB$ (scale-d = 0), or,
  \item $D \leftarrow AB + D$ (scale-d = 1)
\end{itemize}
...
\begin{minipage}[t]{0.48\textwidth}\codeminipage
\vspace{10mm}
{\color{lightttColor}
\begin{verbatim}
...
We introduce \myKeyA{synchronization statements}, which we view as being executed cooperatively by an entire collective lane.

\filbreak
\begin{itemize}
  \item \lighttt{Arrive($A_1$:\textsf{actor kind}, $B$:\textsf{barrier})}
  \item \lighttt{Await($B$:\textsf{barrier}, $A_2$:\textsf{actor kind}, $N$:\textsf{integer} = 0)}
  \item \lighttt{Fence($A_1$:\textsf{actor kind}, $A_2$:\textsf{actor kind})}
\end{itemize}

\filbreak
For \lighttt{Arrive} and \lighttt{Await}, we need $B$ to refer to a barrier variable declared with \lighttt{$B$ : barrier}.

\filbreak
Let $T$ be the set of threads in the collective lane (e.g. CTA) executing the statement.
Each execution of an \lighttt{Arrive} statement defines the first visibility set $V_1 = T \times A_1$ (set of sigthreads).
Each execution of an \lighttt{Await} statements defines the second visibility set $V_2 = T \times A_2$.

\filbreak
Each \lighttt{Await} matches with the $N^{th}$ prior \lighttt{Arrive} done with the same barrier $B$.
All actions performed by a sigthread in $V_2$ subsequent to the \lighttt{Await} may assume that all actions performed by a sigthread in $V_1$ prior to the matched \lighttt{Arrive} have completed and have visible effects.

\filbreak
A \lighttt{Fence($A_1$, $A_2$)} statement is like \lighttt{Arrive($A_1$, $B$)} followed immediately by \lighttt{Await($B$, $A_2$, 0)} where $B$ is an anonymous barrier variable.
...
Futhermore, we'll only support some fixed usage patterns for \lighttt{barrier} declaration, \lighttt{Arrive}, and \lighttt{Await} statements, with the intent that they'll lower in an obvious way to mbarrier or commit\_group synchronization, without surprises.
We'll work out the precise details later.

\filbreak
\mainKey{Barrier Divergence:}
The possibility of barrier divergence is defined out of existence with this design, since the generated barrier will be precisely what is expected for the \myKeyA{collective unit} at the point the synchronization statement was placed.
So for example, \lighttt{Fence(cuda\_classic, cuda\_classic)} lowers in different ways in the following:
{\color{lightttColor}
\begin{verbatim}
with CudaDeviceFunction(blockDim=256):
...
\filbreak
\mainKey{Invalid Instruction Divergence:}
Wherever a call to an \lighttt{instr} is made, we check that the \myKeyA{collective unit} matches that expected for the \lighttt{instr}.
For example, with \lighttt{wgmma\_mma(...)} being a wgmma instruction:
{\color{lightttColor}
\begin{verbatim}
with CudaDeviceFunction(blockDim=256):
    for task in cuda_tasks(...):
