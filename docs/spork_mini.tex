...
\filbreak
\mainKey{Async Blocks (actor kind):}
...
Each async block changes the \myKeyA{actor kind} for its subtree (the proc root has actor kind \lighttt{cpu}).
...
\filbreak
\mainKey{Parallel-for Loops (collective unit):}
...
The new \lighttt{for cuda\_threads} loop changes the collective unit of the subtree.

\filbreak
With the OpenMP-based \lighttt{par} loop, the loop both creates the threads and distributes work among them.
For Spork, the \lighttt{cuda\_threads} loop only subdivides the threads previously launched with \lighttt{CudaDeviceFunction}.
...
% We saw that each statement is executed with a certain actor kind and 
\end{document}
