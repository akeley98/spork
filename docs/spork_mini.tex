...
\begin{enumerate}
  \item In \myKeyA{Exo}, the size is $64 \times 64$, since here, we're concerned about the \textit{dataflow}, so we need the true (logical) tile size.
  \item In \myKeyA{CUDA}, the size is $16 \times 8$, since registers are allocated by threads, and that's the size of the sub-tile for each thread.
\end{enumerate}
