...
\input{ultraspork/cutlass_pseudocode.0.tex}
...
\end{equation*}
Since we store a polynomial for each tensor element, \lighttt{m, n} are in fact constants here, and it's possible to store literal polynomials.
The reason to store them as functions is for de-duplication: we can summarize entire tiles with a single object.
Furthermore, this will match the format by which the output buffers are specified (function of output buffer coordinates to values).
...
\input{ultraspork/mod_heuristic.0.tex}
