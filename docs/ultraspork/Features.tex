...
\begin{itemize}
  \item Retaining Exo-GPU's model of sequential and parallel loops and CUDA kernel launch
  \begin{itemize}
  ...
  \end{itemize}
  \item Tensor variables with ``typed strides''
  \begin{itemize}
  ...
  \end{itemize}
  \item Window references (mutable ``pointer'' variables)
  \item Polynomial-based functional specifications
  \item Annotations on assignment statements that specify, as a function of control variables and the functional specification
  \begin{itemize}
    \item which tensor elements are read (not obvious due to mutable window references)
    \item which tensor element is modified
    \item what values are read
    \item what value is written
  \end{itemize}
  ...
  \item Value correctness checking for concrete problem sizes
