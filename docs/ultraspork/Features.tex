...
\begin{itemize}
  \item Retaining Exo-GPU's model of sequential and parallel loops and CUDA kernel launch (Section~\ref{sec:SeqPar})
  \begin{itemize}
  ...
  \end{itemize}
  \item Tensor variables with ``typed strides'' (Section~\ref{sec:Tensors})
  \begin{itemize}
  ...
  \end{itemize}
  \item Window references (mutable ``pointer'' variables) (Section~\ref{sec:Windows})
  \item Polynomial-based functional specifications (Section~\ref{sec:PolynomialSpec})
  \item Annotations on assignment statements that specify, as a function of control variables and the functional specification (Section~\ref{sec:AssignmentAnnotation})
  \begin{itemize}
    \item the tensor elements referenced by any window references used (Section~\ref{sec:WindowAnnotation})
    \item what values are read (Section~\ref{sec:ReadAnnotation})
    \item what value is written (Section~\ref{sec:WriteAnnotation})
  \end{itemize}
  ...
  \item Value correctness checking for concrete problem sizes (Section~\ref{sec:ValueCorrectnessChecking})
  \item Sync correctness checking like in Exo-GPU
  \item Explicit instruction and procedure calls
\end{itemize}
