...
\begin{document}
\myTitle{Exo GPU -- Spork User Guide \yellowBox{OUTDATED}}
...
In Exo-GPU, each statement is executed with a certain \myKeyA{thread collective}, or grouping of threads guaranteed to have uniform control flow, and with a certain \myKeyA{instruction timeline}.
...
\filbreak
The instruction timeline (\myKeyA{instr-tl}) categorizes instructions based on their synchronization requirements (e.g. CPU, in-order CUDA instructions, and various non-ordered asynchronous CUDA instructions, e.g. cp.async, wgmma, TMA).
...
\filbreak
\lighttt{with CudaAsync(<instr-tl>)}: within a CUDA device function, open a block of async CUDA code, using instructions with the specified instr-tl.
...
This guide contains only a summary of the deduction rules; \lighttt{coll\_algebra.pdf} contains precise documentation.
...
The \myKeyA{tile thread count} of a given scope is the number of threads in the scope's collective unit, \textbf{ignoring} the effects of warp specialization (see \lighttt{coll\_algebra.pdf} for more precise treatment).
...
All instr-tl values have corresponding sync-tl values that express all possible effects of that category of instruction; just drop the \lighttt{\_instr} prefix (e.g. \lighttt{cuda\_in\_order\_instr} $\to$ \lighttt{cuda\_in\_order}).
...
A combination of \lighttt{Arrive(first\_tl, N) >> barrier\_name[...]} and \lighttt{Await(barrier\_name[...], second\_tl, N)} statements express split barriers.
...
There may be array sizes (e.g. \lighttt{[4,2]}) after the \lighttt{barrier} type name.
...
We also need to document the sync timeline values, and implementable usage of various barriers (most notably, the mbarrier pairing requirement -- see \lighttt{summer\_of\_wgmma.pdf}, ``Exo mbarrier Pairing Requirement'').
...
\filbreak
\mainKey{Queue Barrier:} In our abstraction, each barrier variable names a queue barrier array, with each indexed position holding a \myKeyA{queue barrier}.
For mbarriers only, we further distinguish between the ``front queue barrier array'' (\lighttt{+ <name>}) and ``back queue barrier array'' (\lighttt{- <name>}); both associated with the same variable name.
...
TODO: this seems bizarre at first glance; explain (it's related to the mbarrier pairing requirement in \lighttt{summer\_of\_wgmma.pdf}).

\filbreak
\lighttt{Arrive} and \lighttt{Await} statements that use the same queue barrier pair with each other, with delay controlled by the \lighttt{N} parameter. We currently require $N = 1$ for arrive statements.
...
\mainSub{With Statements}

All with statements (excluding those defined by metaprogramming) are interpreted as \lighttt{if True:} in S-semantics (which scheduling rewrites use), \textit{except}, \lighttt{simplify} does not optimize them out.

\input{guide/with_grammar.0.tex}
\filbreak
\mainSub{For Loops}

\input{guide/for_grammar.0.tex}
\filbreak
\mainSub{Collective Units}

\input{guide/coll_grammar.0.tex}
\filbreak
\mainSub{Synchronization}

\input{guide/sync_grammar.0.tex}
\filbreak
\mainSub{Misc}

\input{guide/misc_grammar.0.tex}
\vfill
...
\newpage
\myTitle{Example with mbarrier}

This is similar to the previous sm\_80 GEMM, except we replaced the \lighttt{Fence} statement with mbarrier synchronization, and generalized the double buffer to a ring buffer (of depth given by the \lighttt{RING} constant).

\input{guide/xgemm_Sm80_mbarrier.0.tex}

\newpage
\mainSub{mbarrier Code Detail}

Besides replacing the synchronization statements, we just have to change the SMEM ring buffer indexing logic.

\input{guide/xgemm_Sm80_mbarrier.1.tex}
\filbreak
\input{guide/xgemm_Sm80_mbarrier.2.tex}
