...
Note that the read and write visibility sets are \textit{not} ``allowed to read'' and ``allowed to write'' sets.
...
\filbreak
After issuing these directives, the user must ensure that the program, when dynamically executed on the abstract machine, meets the visibility requirements of the abstract machine, at least for the concrete problem size they care about.
