...
For example, the mbarrier for TMA GMEM$\to$SMEM is \texttt{Arrive(\myKey{tma\_to\_shared}, <barrier>)},\\\texttt{Await(\myKeyB{cuda\_generic}, <barrier>)}.
...
We now define that read/write is \myKeyB{visible-to} sigthread $(t, a)$ if the $(t,a) \in V_S$ ($V_S$ being the sync visibility set of the read/write visibility record).
