...
\newpage
\begin{minipage}[t]{0.5\textwidth}\fixminipage
\myTitle{Synchronization Environment}  % moved into minipage to fit
...
Each synchronization statement defines a \myKey{``first visibility set''} $V_1$ and \myKey{``second visibility set''} $V_2$;
...
\begin{itemize}
  \item $Q$ \myKeyB{synchronizes-with} the fence if $Q \cap V_1$ is nonempty
  \item If so, $Q \leftarrow Q \cup V_2$
\end{itemize}
\end{minipage}

\newpage
\myTitle{Synchronization Checking -- Simplified}
...
We say that the write recorded by $W$ is \myKeyB{visible-to} $t$.
...
We say that the read recorded by $R$ is \myKeyB{visible-to} $t$.
...
So our previous formalization \myKey{won't work} for these.
...
\end{itemize}
Where $D$ (accumulator) is stored distributed in the 128 threads' \textbf{registers}, $B$ is stored in \myKeyB{shared memory}, and $A$ may be in either format.
...
For \myKeyB{shared memory}, we need to issue an async proxy fence, unless the memory was written by TMA (which is already in the async proxy).
...
\newpage
\myTitle{Abstraction: Actor Signature}

\begin{minipage}[t]{0.5\textwidth}\fixminipage
Conceptually, the earlier sychronization checking model was based on analyzing a \textit{sequential} trace of the program, where each \myKey{action} is annotated by the \myKey{``actor''} (thread index) of ``who'' executed the action.

We can extend this annotation to capture ``how'' this thread performed the action.
This is the \myKey{actor signature}.
...
\newpage
\myTitle{Exo Features for Async CUDA}
...
\begin{itemize}
  \item \emphtt{wgmma\_async}: \{\texttt{sig\_wgmma\_mem, sig\_wgmma\_reg\_a, sig\_wgmma\_reg\_d}\}
  \item \emphtt{cuda\_sync}: \{\texttt{sig\_cuda\_sync}\}
  \item \emphtt{cuda\_generic}: Both sync and async instructions in the generic proxy, i.e. not needing a proxy fence.
\end{itemize}
...
\texttt{\ \ \ \ Fence(\myKey{cuda\_sync}, \myKeyB{cuda\_generic})} \\
Because the sync only waits for synchronous instructions \myKey{before} the barrier, but any instruction in the generic proxy \myKeyB{after} the barrier can safely read the protected values.
...
\texttt{Arrive} is parameterized with a \myKey{before} actor kind, \texttt{Await} with an \myKeyB{after} actor kind.

For example, the mbarrier for TMA GMEM$\to$SMEM is \texttt{Arrive(\myKey{tma\_to\_shared}, <barrier>)},\\\texttt{Await(\myKeyB{cuda\_all}, <barrier>)}.

Implement only supported arrive/await patterns.

\mySub{Lane Specialization}
...
\end{verbatim}
\myKey{S-semantics:} treat like \texttt{if True:}
\end{minipage}

\newpage
\myTitle{Synchronization Checking -- Fuller Story}

Sketch of changes to the model needed -- again, see spork.pdf for details.

\begin{minipage}[t]{0.5\textwidth}\fixminipage

\mySub{Sigthreads (thread index, actor signature)}

Sigthreads replace thread indices in visibility sets.

Each action is performed by an \myKey{accessor set} $T \times \{A_O\}$,
$A_O$ being the \myKey{actor signature} of the access and $T$ being the
set of thread indices (could be more than 1 for warp/warpgroup instructions).

\mySub{Visibility Record}

The read/write visibility sets (per variable in the synchronization environment) are replaced with read/write \myKey{visibility records}, consisting of
\begin{itemize}
...
\hfill
\begin{minipage}[t]{0.5\textwidth}\fixminipage
\mySub{Read/Write checking}

We now define that read/write is \myKeyB{visible-to} sigthread $(t, a)$ if $(t,a) \in V_S$ ($V_S$ being the sync visibility set of the read/write visibility record).

\mySub{Synchronization Statements}

For \texttt{Fence($A_1$, $A_2$)},$$V_1 = T \times A_1, V_2 = T \times A_2$$
$T$: executing thread indices, $A_1,A_2$: actor kinds

We define that a read/write visibility record \myKeyB{synchronizes-with} an executed fence if $V_A \cap V_1$ is non-empty.
If so,
\begin{itemize}
