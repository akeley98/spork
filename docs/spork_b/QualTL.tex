...
\texttt{cuda\_mbarrier\_qual} (mbar): usage of mbarriers (Note, ``full'' in \texttt{cuda\_temporal} is intentional)\\
\texttt{Sm80\_cp\_async\_qual} (Sm80): accessed by \lighttt{cp.async} instruction (non-bulk, i.e. not TMA)\\
\texttt{tma\_to\_smem\_async\_qual} (g2s): accessed by \lighttt{cp.async.bulk} ``load'' instruction (GMEM$\to$SMEM)\\
\texttt{tma\_to\_gmem\_async\_qual} (s2g): accessed by \lighttt{cp.async.bulk} ``store'' instruction (SMEM$\to$GMEM)\\
