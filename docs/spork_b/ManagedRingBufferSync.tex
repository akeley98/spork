...
This is enforced in the abstract machine by adding a special \textsf{VisRecord} upon allocation (Section~\ref{sec:SyncSemanticsManagedRingBuffer}).

\mainKey{Arrive Requirement:} If a free-on-arrive barrier exists for $x[j^*]$, must arrive on the free-on-arrive barrier after all reads, mutates, and sync statement instances referencing $x[j^*]$.
Executed statements must not reference $x[j^*]$ after the arrive.
This is enforced in sync-check by abstract machine semantics upon free (Section~\ref{sec:ChecksOnFree}).
