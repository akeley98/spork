...
Each visibility record $r$ is a tuple of type $\mathcal{P}(\mathsf{TlSig}) \times \mathcal{P}(\mathbb{P}) \times \mathcal{P}(\textsf{QualTL})$, with the elements being
\begin{itemize}
  ...
  \item The \myKeyA{pending await} function (def~\ref{sec:gPendingAwait}), denoted $r.p$.
  This is a function of type $\mathbb{B} \times \mathbb{Z}^* \to \mathbb{Z} \cup \{\infty\}$, which maps barrier array elements to a ``minimum arrive count''.
  This mediates the interaction between paired \lighttt{Arrive} statement instances (Section~\ref{sec:ArriveSemantics}) and \lighttt{Await} statement instances (Section~\ref{sec:AwaitSemantics}).
  \item The \myKeyA{qualitative timeline mask} (def~\ref{sec:gQualTLMask}), denoted $r.Q$, which comes from the memory type or barrier mechanism.
\end{itemize}
...
The sync-check error messages format a visibility record's visibility set and pending await function separately.
...
\end{itemize}
Each pending await $((z, n^*), a)$ with $a \ne \infty$ in the pending await function is formatted as \\
``\texttt{pending await: $z$[$n^*$] arrive\_count=$a$}''.\\
Issue, these are sorted based on internal hashing, and not anything meaningful.
