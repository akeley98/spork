...
For each collective indexing pair $(\omega_0, e^*)$, we deduce a \myKeyA{thread pitch tuple}.
The deduced number of distributed dimensions ($R$) is the length of this tuple.
All collective indexing pairs in the input set must lead to the same deduced tuple.

We will soon define what a \myKeyA{required iterator} is.
...
A collective index pair $(\omega_0, e^*)$ is first updated with domain completion (def~\ref{sec:gDomainCompletion}) so that the memory type's collective type ($\delta_0$) has the same domain as the collective tiling.
