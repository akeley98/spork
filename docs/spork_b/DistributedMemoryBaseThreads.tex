...
Given a collective tiling $\omega_0$ and the collective type $\delta_0$ unpacked from the native unit (def~\ref{sec:gNativeUnit}), let $(\omega, \delta) = \mathsf{domainCompletion}(\omega_0, \delta_0)$.
...
\mainKey{Requirement:} Any two collective indexing pairs in the input set must pass the base threads requirement: their collective tilings must satisfy base offset equivalence and base box equivalence.
