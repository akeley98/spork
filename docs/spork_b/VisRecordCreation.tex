...
(For the special \textsf{VisRecord} created for managed ring buffers, see Section~\ref{sec:SyncSemanticsManagedRingBuffer})
...
Instruction call instances (Section~\ref{sec:InstrMemoryAccess}) are decomposed to separate read and mutate effects.
...
\begin{itemize}
  ...
  \item the qualitative timeline mask $Q$, queried from the memory type or barrier mechanism of the array element referenced.
\end{itemize}

Define $r = \mathsf{NewVisRecord}(g^*)$ as having a visibility set $r.s$ that is the union of
\begin{itemize}
...
\end{itemize}
and a pending await function $r.p$ that is based off the arrive count (Section~\ref{sec:SyncEnv}) of each referenced barrier array element:
\begin{equation*}
    r.p(z, n^*) = \begin{cases}
      \rho(z, n^*).b.a & \text{ if } z[n^*] \text{ is referenced (def~\ref{sec:gBarrierExpr}) in the trailing barrier expression } \\
      \infty & \text{ otherwise (always the case, if there's no trailing barrier expression)}
    \end{cases}
\end{equation*}
and qualitative timeline mask $r.Q = Q$.
