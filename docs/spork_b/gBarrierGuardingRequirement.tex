...
\mainKey{Requirement 2/3:} Each collective indexing pair (Section~\ref{sec:CollIndexingPairs}) collected from \lighttt{Await} statements for $z_g$ must pass the base threads requirement (Section~\ref{sec:DistributedMemoryBaseThreads}) with each collective indexing pair collected from \lighttt{Arrive} statements for $z_a$.
Furthermore, the thread pitch tuple (Section~\ref{sec:DistributedMemoryThreadPitch}) deduced from the \lighttt{Await} statements for $z_g$ must be equal to the thread pitch tuple deduced from the \lighttt{Arrive} statements for $z_a$.
