...
\newpage
\myBiggerTitle{GEMM: Starting out with Exo \yellowBox{I am working on a new DSL}}
...
\node (grid) [normalnode, nGreenstyle, text width=5cm, minimum width=5cm] {\textbf{grid}};
...
\node (thread0) [normalnode, nRedstyle, text width=16mm, minimum width=16mm, below=of cta0, yshift=-1cm, xshift=-32mm] {\textbf{thread}};
\node (thread1) [normalnode, nRedstyle, text width=16mm, minimum width=16mm, right=of thread0] {\textbf{thread}};
\node (thread2) [normalnode, nRedstyle, text width=16mm, minimum width=16mm, right=of thread1, xshift=22mm] {\textbf{thread}};
...
{\LARGE
TODO: divide \blueBox{\texttt{k}} loop \& reorder outside \texttt{\textbf{cuda\_threads}} loops.
\begin{itemize}
  \item Enables thread cooperation inside \blueBox{\texttt{k1}} loop.
\end{itemize}
...
\yellowBox{Skip scheduling to here, boxed main loop before and k1 here too and also A[...,k]*B[k,...]}
...
Replace a block of code with a \myKeyA{semantically-equivalent sub-procedure} (blah blah ignore parallelism)
...
\hfill \texttt{Arrive(\myKeyA{L1}, 1) >> \textit{var}} \hfill \texttt{Await(\textit{var}, \myKeyB{L2}, \blueBox{\textsf{delay}})} \hfill
...
Match pairs of executed Arrives \& Awaits with the same barrier variable, adjusted by \blueBox{delay}.
%% Arrive/Await matching:
%% \begin{itemize}
%%   \item Match pairs of executed \texttt{Arrive}/\texttt{Await} with the same \texttt{barrier} variable.
%%   \item Adjust matching based on \blueBox{delay} value.
%% \end{itemize}
