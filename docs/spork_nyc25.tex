...
{\LARGE
Phase 1: apply scheduling operators
\begin{itemize}
  \item Analyzes \myKeyB{parallel} loops as if they were \myKeyA{sequential}.
\end{itemize}

Phase 2: sync check final \texttt{proc}
\begin{itemize}
...
\begin{tikzpicture}[node distance=8mm]
\input{nyc25_exo_spork_flow.tex}
\node (sequential) [below=of exo, yshift=6mm, align=center] {\textbf{Sequential}};
\node (am) [below=of sync, yshift=6mm, align=center] {\textbf{Abstract Machine}};
\node (semantics) [left=of sequential] {Semantics used:};
\end{tikzpicture}
}
\end{center}
