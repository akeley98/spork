...
\begin{itemize}
  \item Thread count \& ``shape'' defined by \myKeyA{collective unit}.
\end{itemize}
...
\newpage
\myBiggerTitle{Summary: Concurrency Forms}

{\LARGE
Exo-GPU added features:
\begin{itemize}
  \item expressing cross-thread \myKeyB{parallelism}
  \item expressing intra-thread \myKeyB{concurrency}\\(async instructions \& \myKeyA{timelines})
\end{itemize}
...
\textbf{Same as Exo:} value effects (read/write)
\begin{itemize}
  \item Numeric values
\end{itemize}

\textbf{New:} visibility effects (read/write/\myKeyA{sync stmt})
\begin{itemize}
  \item Sets of (\myKeyA{timeline} $\times$ thread) involved\\as def'd by parallel loops, async instrs, etc.
\end{itemize}

Hybrid approach: sequential execution, \myKeyB{parallel metadata}.
...
\LARGE
\begin{tikzpicture}[node distance=0mm]
\input{nyc25_visibility_value.tex}
\end{tikzpicture}
\end{center}
...
{\LARGE
\textbf{Write/Read Pair}: read of a value given by sequentially-prior write.

\blueBox{\textbf{Value Effects:}} literally the same as sequential Exo.

\greenBox{\textbf{Visibility Effects:}} check sufficient syncs.% stmts in between.

\begin{tikzpicture}[node distance=0mm]
\node (write) [align=center] {Write (\myKeyA{timeline} $\times$ thread)};
\node (read) [align=center, right=of write, xshift=24mm] {Read (\myKeyA{timeline} $\times$ thread)};
\draw [arrow] (write) -- node[above] {\large \textbf{SYNC?}} (read);
\node (dummy) [left=of write, xshift=-12mm] {~};
\end{tikzpicture}
...
%% {\LARGE
%% \blueBox{\textbf{Value Effects}}: Trivial \myKeyA{functional equivalence}
%% \begin{itemize}
%%   \item $p_n$ as abstract machine proc $\equiv$ $p_n$ as sequential Exo proc.
%% \end{itemize}

%% \greenBox{\textbf{Visibility Effects:}} If checks pass,
%% \begin{itemize}
%%   \item each \myKeyA{Write/Read Pair} in the sequential baseline
%%   corresponds to a correctly-sync'd \myKeyA{Write/Read Pair} in the parallel hardware.
%% %  \item ... same for Read/Write, Write/Write.
%% \end{itemize}
%% }
%% \vfill
%% \begin{center}
