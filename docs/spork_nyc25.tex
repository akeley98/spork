...
\newpage
\myBiggerTitle{GEMM: Starting out with Exo \hfill \nBlueBox{GEMM v1}}
...
\newpage
\myBiggerTitle{GEMM: Type Annotations \hfill \nBlueBox{GEMM v1}}
...
\newpage
\myBiggerTitle{GEMM: Memory Type Annotations \hfill \nBlueBox{GEMM v1}}
...
\newpage
\myBiggerTitle{GEMM: M/N Tiling \hfill \nBlueBox{GEMM v1}}
...
\newpage
\myBiggerTitle{GEMM: M Variable Rewritten}
...
\newpage
\myBiggerTitle{GEMM: Assume Perfect Tiling}
...
\begin{itemize}
  ...
  \item New ``parallelize loop'' rewrites are trivially correct\\(at this stage).
\end{itemize}

Parallelism checking is made orthogonal to rewrites.
...
\newpage
\myBiggerTitle{Scheduling Chain of Equivalence}
...
\begin{itemize}
  \item Parallelized $p_n$ functionally equivalent to sequential $p_n$.
\end{itemize}
}
...
\node(procNM) [normalnode, right=of procNS, text width=4cm, minimum width=4cm] {Final proc $p_n$,\\\myKeyB{parallel}\\interpretation};
...
\draw [line, draw=blueBoxFg] (sync.north) to[out=135, in=270] (procNS.south);
\draw [line, draw=blueBoxFg] (sync.north) to[out=45, in=270] (procNM.south);
\draw [line, draw=redBoxFg] (note) -- (procNS);
\draw [line, draw=redBoxFg] (note) -- (procNM);
\draw [line, draw=blueBoxFg] (spork.north) to[out=135, in=270] (procNM.south);
\draw [line, draw=blueBoxFg] (spork.north) to[out=45, in=270] (cuda.south east);
