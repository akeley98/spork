...
\magicSubsection{Basic Language}{sec:BasicLanguage}

Sequential language with extensions for launching CUDA kernels and parallelizing loops.

\mainKey{Typing:}
Control and data types, tensors, windows, resident in some memory type.

\mainKey{Familiar stuff:} Sequencing, sequential loop, if/else, read, write, reduce (\lighttt{+=})

\mainKey{Cuda Device Function Launch:} \lighttt{with CudaDeviceFunction(...)}.
CUDA scope inside; CPU scope outside

\mainKey{Parallel Loops:} \lighttt{for \_ in cuda\_tasks(\_, \_)} assigns iterations to clusters.
\lighttt{for \_ in cuda\_threads(\_, \_, unit)} assigns iterations to threads within a cluster.
A typed group of threads (e.g. 1 thread, 1 warp, 1 warpgroup) may be assigned per iteration, as defined by \lighttt{unit}.

\mainKey{Data Allocation:} Name, base type, extents of tensor, memory type.
Memory type fuses together issues of physical memory type (RMEM, SMEM, GMEM), allocation method, data layout/swizzling.
Future work may orthogonalize these concerns.

\mainKey{Instr Call:} Function-like wrapper around PTX instructions.

\mainKey{Omit:} CPU procs. Distraction from instrs with subtly different semantics.

\mainKey{Difficult Concepts:}
Atomics, split barrier (arrive/await), SMEM allocation details, multicast, TMA/SpecialWindow, warp specialization.
Defer discussion of these features until additional sections at the end of paper.

\mainKey{Window Statement:} (defer)

\mainKey{with CudaWarps:} (defer)

\mainKey{Synchronization:} Barrier allocation, Fence, Arrive, Await (defer)

\magicSubsection{CUDA Kernel Form}{sec:CudaKernelForm}

The body of a \lighttt{CudaDeviceFunction} must be a single \lighttt{cuda\_tasks} loop.
Each \lighttt{cuda\_tasks} loop must have either one \lighttt{cuda\_tasks} loop nested inside,
or contain the GPU code to be executed per-cluster: the \myKeyA{device task}.
\lighttt{cuda\_threads} loops may only appear in a device task.
\lighttt{cuda\_tasks} loops may not appear anywhere other than as defined above.

\magicSubsection{Thread Mapping}{sec:ThreadMapping}

\mainKey{Thread ID:} Pair of task ID and local thread index.
Task ID distinguishes threads in different clusters
(abstractly, assigned to different device task instances).
Local thread index is \lighttt{cluster\_ctarank * blockDim.x + threadIdx.x}.

\mainKey{Thread Collective:} Set of thread IDs,
representing a set of threads assigned to cooperatively execute some
statement. Note, this cooperation does not imply lockstep / implicit synchronization.

\mainKey{Collective Types:} Thread, warp, warpgroup, etc.;
each thread collective may match 0 or more collective types.

\mainKey{Thread Mapping Function:} Annotates each statement in a device task
(statements within an innermost \lighttt{cuda\_tasks} loop).
Gives the thread collective assigned to execute an instance of a statement
as a function of the environment (specifically, the parallel loop iterator variables).

In principle, each live allocation should also be considered annotated with a similar thread mapping function,
this one mapping tensor coordinates to the thread(s) that own the physical memory for storing the identified tensor element.

\magicSubsection{Accelerator Instructions \& Effects}{sec:Instr}

\magicSubsection{Static Correctness Checks}{sec:StaticCorrectness}

\magicSubsection{Synchronization Checking}{sec:SyncCheck}

Reason about the execution traces.
