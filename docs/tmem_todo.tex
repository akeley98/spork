...
Check out and branch off the \lighttt{akeley98/camspork} branch.
...
Since the TMEM pointers are not like the C pointers that Exo has built-in support for, you will have to wrap your memory types with \lighttt{@window\_indexer} to customize the compilation of TMEM pointer expressions (32-bit integers AFAIK).
I recommend not using the old \lighttt{MemWin.window} callback.
You can look at \lighttt{exo.spork.cuda\_memory.CudaRmemPacked32} or \lighttt{exo.platforms.Sm90.Sm90\_SmemSwizzled} for example code.
