...
Why did the triangle mesh approach benefit from occlusion culling?

\begin{itemize}
  \item Early-z skips fragment shaders, not vertex shaders
  \item Have to rasterize triangles to know the tiles covered
  \item We only waste 12 triangles (AABB) when culled
...
\begin{itemize}
\item Uses DMA engine (Vulkan transfer queue) to copy raw voxels from file to 3D texture (faaast)
\item Calculates new AABB (faaast)
\end{itemize}
\vspace{4mm}
