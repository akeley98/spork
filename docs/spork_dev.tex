...
wgmma synchronization only occurs within a single warpgroup.
...
\item \texttt{wgmma.wait\_group N} waits for the MMAs of the $N^{th}$ prior pipeline stage (0-indexed)
\end{itemize}
...
\item \myKey{GMEM predication:} reads get default value; writes discarded
\end{itemize}
...
\begin{itemize}
  ...
  \item Used on device, but must be encoded on the host CPU (not 100\% true)
\end{itemize}
...
Replaces \texttt{GMEM[\textit{slice}] = SMEM} with\\
\texttt{GMEM[\textit{slice}] += SMEM}
