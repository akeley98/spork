...
\end{itemize}
\vspace{8mm}
The SyncEnv will track the ``visibility'' (set of threads that can see) for \textit{each} read and write done to each variable -- this \myKey{changes with synchronization}
...
\begin{itemize}
  ...
  \item assumes \myKey{mutable access} must be serialized
\end{itemize}
...
\newpage
\myTitleExtra{Extra Discussion Slide: Rewrite Rule Issues (specials cases for value 0)}
