...
{ \LARGE Exo Dev 2025-01-15 } \yellowBox{(updated 2025-04-17)} \hfill { \LARGE \textbf{\textsf{Spork: EXO GPU}}}
...
(\yellowBox{update:} may have to make changes)
...
\begin{itemize}
  ...
  \end{itemize}
  \item \texttt{for i in cuda\_threads(lo,hi,unit=...)}\\ = distribute work across (grouped) threads
...
\begin{itemize}
  ...
  \item \texttt{for i in cuda\_threads(lo,hi)}\\ = sequential for loop
...
\begin{itemize}
  \item Easy to synchronize within CTAs; hard between CTAs (no Exo support except clusters)
...
\end{itemize}
\texttt{wgmma.wait\_group N} waits for the MMAs of all but the last $N$-many commit-groups
...
\begin{itemize}
  ...
  \item Used on device, but should be encoded on the host CPU
  \begin{itemize}
    \item not 100\% true (tensorMap proxy)
  \end{itemize}
\end{itemize}
...
\node(c12) [smallnode, below=of c02, fill=violetBoxBg, draw=violetBoxFg, text=violetBoxFg] {$C_{1,2}$};
...
\hfill
\begin{minipage}[t]{0.48\textwidth}\fixminipage
We define two levels of parallel for loops:
\begin{itemize}
  \item \texttt{for \_ in cuda\_tasks(lo, hi):}\\assign ``tasks'' to CTAs or clusters

  For now, we launch as many clusters as will fit the device, and assign tasks to clusters by round-robin scheduling (persistent kernel)

  \item \texttt{for \_ in cuda\_threads(lo, hi, unit):}\\assign loop iterations to different \myKeyA{collective lanes} within the cluster/CTA

  The number of threads and ``shape'' of the collective lane is defined by the (collective) \texttt{unit}
\end{itemize}

\yellowBox{TODO:} consider standardizing on Descend ``execution resource \{type\}'' terminology?

\yellowBox{TODO:} precise specification of how the \texttt{unit} parameter works.
In particular, complications from the fact that clusters introduce non-hierarchical work decomposition, e.g., we could execute some code with 1 warp chosen from each CTA of a cluster.

\end{minipage}
\newpage
\myTitle{Exo Syntax -- Warp Specialization}
...
with CudaDeviceFunction(blockDim = 5 * 128):
    for b in cuda_tasks(...):
        ...
        for wg in cuda_threads(0, 5, unit=128*cuda_thread):
...
\end{verbatim}
Nest inside \myKey{\texttt{with CudaWarps(lo, hi)}} to override this; assign \myKey{different work} to \myKey{different warps}
\begin{verbatim}
with CudaDeviceFunction(blockDim = 5 * 128):
    for b in cuda_tasks(...):
        with CudaWarps(0, 4):        # Overloaded with statement
            for wg in cuda_threads(0, 1, unit=128*cuda_threads):
                # This code mapped to warpgroup 0
        with CudaWarps(4, 20):
            for n_wg in cuda_threads(0, 2, unit=256*cuda_threads):
                for m_wg in cuda_threads(0, 2, unit=128*cuda_threads):
...
with CudaDeviceFunction(blockDim=256):
    for b in cuda_tasks(...):
        # Executing collective lane is CTA
...
\hfill
\begin{minipage}[t]{0.48\textwidth}\fixminipage
Ignore \texttt{cuda\_classic} for now (actor kind, to be explained)
...
\begin{itemize}
  \item \texttt{Sm80\_cp\_async}
  \begin{itemize}
    \item Ampere async copy\\(uses generic proxy; not discussed)
  \end{itemize}
  \item \texttt{tma\_to\_smem\_async}
  \item \texttt{tma\_to\_gmem\_async}
...
\begin{verbatim}
  Fence(A1, A2)
\end{verbatim}
Within the executing collective lane, prior actions of actor kind \texttt{A1} are visible to subsequent actions of actor kind \texttt{A2}

\mySub{Split Barrier} \yellowBox{(somewhat out of date)}
...
Actions of actor kind \texttt{A1} prior to the $N^{th}$ arrive are visible to actions of actor kind \texttt{A2} subsequent to the $N^{th}$ await.
...
e.g. \texttt{Sm80\_generic} (generic proxy)

\end{minipage}

\newpage
\myTitleExtra{Extra Info: CudaAsync Block}
...
\begin{itemize}
  ...
  \item Changes actor kind from \texttt{cuda\_classic} to \texttt{\textit{actor\_kind}}
\end{itemize}
...
%% \newpage
%% \myTitleExtra{Extra Info: Syntax Example}
%% \begin{verbatim}
...
\newpage
\myTitleExtra{Extra Info: Contrast with prior work: Triton}
...
\node(procNS) [normalnode, right=of proc1, xshift=+6mm] {$proc_N$\\\myKeyA{S-semantics}};
...
\node(caption) [widenode, left=of note] {``chain of equivalence''};
...
\begin{itemize}
  ...
  \item Formal: \myKey{set} of allowed \myKey{actor signatures}\\\yellowBox{(out of date: more complicated)}
...
\vspace{-4mm}
\begin{align*}
    & \greenBox{\textit{[[ thread 0 kicks off TMA ]]}} \\
    ...
    & \violetBox{\textit{[[ thread $x$ reads SMEM[$x$] ]]}} \\
...
\begin{itemize}
  \item $V_S$ = $\{(t,a)\}$ if the instruction's access is synchronous, otherwise $\{\}$
  \item $V_A$ = $\{(t,a)\}$
  \item $A_O$ = $a$
...
\begin{itemize}
  \item \yellowBox{TODO} specify correctness requirements
...
\violetBox{\texttt{Fence(cuda\_classic, cuda\_classic)}}
\begin{itemize}
  \item $V_1 = [256, 511] \times \texttt{cuda\_classic}$
  \item $V_2 = [256, 511] \times \texttt{cuda\_classic}$
\end{itemize}
\end{minipage}
\hfill
...
\node(b0) [widenode, right=of gmem0, xshift=6mm, fill=redBoxBg, draw=redBoxFg, text=redBoxFg] {$V_S,V_A = [0,255] \times \texttt{cuda\_classic}$\\\textit{\color{lightttColor}reads from previous iter $b=0$}};
\node(r256) [widenode, below=of b0, fill=redBoxBg, draw=redBoxFg, text=redBoxFg] {$V_S,V_A = \{ (256, \texttt{sig\_cuda\_classic}) \}$};
\node(r511) [widenode, below=of r256, fill=redBoxBg, draw=redBoxFg, text=redBoxFg] {$V_S,V_A = \{ (511, ''~'') \}$};
\node(w256) [widenode, below=of r511, fill=blueBoxBg, draw=blueBoxFg, text=blueBoxFg] {$V_S,V_A = \{ (256, ''~'') \}$};
\node(w511) [widenode, below=of w256, fill=blueBoxBg, draw=blueBoxFg, text=blueBoxFg] {$V_S,V_A = \{ (511, ''~'') \}$};
...
\node(b0) [widenode, right=of gmem0, xshift=6mm, fill=redBoxBg, draw=redBoxFg, text=redBoxFg] {$V_S,V_A = [0,255] \times \texttt{cuda\_classic}$\\\textit{\color{lightttColor}unchanged as $V_A \cap V_1$ empty}};
\node(r256) [widenode, below=of b0, fill=redBoxBg, draw=redBoxFg, text=redBoxFg] {$V_S,V_A = [256, 511] \times \texttt{cuda\_classic}$\\\textit{\color{lightttColor}read visibility set unioned with $V_2$}};
\node(w256) [widenode, below=of r256, fill=blueBoxBg, draw=blueBoxFg, text=blueBoxFg] {$V_S,V_A = [256, 511] \times \texttt{cuda\_classic}$\\\textit{\color{lightttColor}write visibility set unioned with $V_2$}};
...
\begin{verbatim}
            # fence.proxy.async
            Fence(cuda_classic, tma_to_gmem_async)
\end{verbatim}
\end{mdframed}
\begin{mdframed}[style=MyFrame, backgroundcolor=greenBoxBg]
\color{greenBoxFg}
...
\violetBox{\texttt{Fence(cuda\_classic, tma\_to\_gmem\_async)}} has
\begin{itemize}
  \item $V_1 = \{(256, \texttt{sig\_cuda\_classic})\}$
  \item $V_2 = \{(\mathbf{256}, \textbf{\texttt{sig\_tma\_to\_gmem}})\}$
  \item Note, \texttt{sig\_cuda\_classic} $\in$ \texttt{cuda\_classic}
\end{itemize}
\blueBox{$(256, \texttt{sig\_cuda\_classic}) \in [256, 511] \times \texttt{cuda\_classic}$}
\end{minipage}
\hfill
...
\begin{verbatim}
            ...
            Fence(tma_to_gmem_async, cuda_classic)
\end{verbatim}
\end{mdframed}
...
\begin{itemize}
  ...
  \item $V_2 = \{(\mathbf{256}, \textbf{\texttt{sig\_cuda\_classic}})\}$
\end{itemize}
...
\node(incoming256) [normalnode, text width=4cm, above=of smem0, yshift=6mm, fill=greenBoxBg] {Incoming write\\$(t,a) = (256,$\\\texttt{sig\_cuda\_classic}$)$};
\node(incoming511) [normalnode, above=of smem255, yshift=6mm, fill=greenBoxBg] {Incoming write\\$(t,a) = (511,$``~''$)$};

\draw[dotted] (incoming256) to (incoming511);
\draw[arrow] (incoming256) to (smem0);
\draw[arrow] (incoming511) to (smem255);

\node(rtma) [widenode, below=of smemdots, fill=redBoxBg, draw=redBoxFg, text=redBoxFg, yshift=-10mm] {$V_S = \{(\mathbf{256}, \textbf{\texttt{sig\_cuda\_classic}})\}$\\$V_A=\{(256, \texttt{sig\_tma\_to\_gmem}),$ $(\mathbf{256}, \textbf{\texttt{sig\_cuda\_classic}})\}$\\\textit{\color{lightttColor}$V_A,V_S$ unioned with $V_2$}};
...
\greenBox{$(t,a)$} $\notin$ \redBox{$\{(256, \texttt{sig\_cuda\_classic})\}$}\\
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Top-level loops within \texttt{CudaDeviceFunction} block must be \myKey{\texttt{cuda\_tasks}}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item $A$ = \texttt{Sm80\_cp\_async}: Ampere async memcopy
  \end{itemize}
...
\newpage
\myTitleExtra{Extra Discussion Slide: Memory Types \& Kernel Launch} \hfill \yellowBox{(OUT OF DATE)}
...
%%     for yi in cuda_threads(0, 16, unit=16*...):
...
%% Map \texttt{cuda\_tasks} iteration space to
%% \begin{itemize}
...
\begin{verbatim}
for io in cuda_tasks(0, ...):  # outer loop
