...
One write visibility record for recent write, and one read visibility record added for \myKey{each} previous read
...
\node(reads) [normalnode, below=of vars, xshift=-25mm, minimum width=4cm, text width=4cm, draw=redBoxFg, text=redBoxFg, fill=redBoxBg] {read visibility records};  % Red/Read
\node(writes) [normalnode, below=of vars, xshift=+25mm, minimum width=4cm, text width=4cm, draw=blueBoxFg, text=blueBoxFg, fill=blueBoxBg] {write visibility record};
...
\hfill
{\color{titleColor}Ignore split barriers (\texttt{Arrive}, \texttt{Await}) for today; see spork.pdf}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item set of sigthreads ``before the fence''
  \end{itemize}
  \item Define \myKey{second visibility set}: $V_2 = T \times A_2$
  \begin{itemize}
    \item set of sigthreads ``after the fence''
  \end{itemize}
\end{itemize}
The effect of a \texttt{Fence} applies to \myKey{each visibility record} in the entire SyncEnv that \myKeyB{synchronizes with} (tbd) the fence.
For each such record,
\begin{itemize}
  \item $V_A \leftarrow V_A \cup V_2$
  \item $V_S \leftarrow V_S \cup V_2$
  \item Expensive to simulate literally!
\end{itemize}
A fence can be \myKey{transitive} or \myKey{non-transitive}
\begin{itemize}
  \item Transitive: \myKeyB{synchronizes with}\\means $V_A \cap V_1$ non-empty
  \item Non-transitive: \myKeyB{synchronizes with}\\means $V_A \cap V_1 \cap (\mathbb{N} \times A_O)$ non-empty
  \begin{itemize}
...
\newpage
\myTitle{SyncEnv Correctness Checking}
%% \newpage
%% \myTitle{Example GPU gemm}

%% \begin{minipage}[t]{\textwidth}\fixminipage
%% \begin{verbatim}
%% # DRAM is not correct, should be CudaGmem
%% def exo_cuda_gemm(M: size, N: size, K: size, A: f32[M, K] @ DRAM,
%%                   B: f32[K, N] @ DRAM, C: f32[M, N] @ DRAM):
%%     assert M % 16 == 0
%%     assert N % 16 == 0
%%     assert K % 8 == 0
%%     # (M/16) x (N/16) grid of 256-thread blocks
%% \end{verbatim}
%% \begin{mdframed}[style=MyFrame, backgroundcolor=greenBoxBg]
%% \color{greenBoxFg}
%% \begin{verbatim}
%%     with CudaDeviceFunction(256, 1):
%% \end{verbatim}
%% \end{mdframed}
%% \begin{mdframed}[style=MyFrame, backgroundcolor=yellowBoxBg]
%% \color{yellowBoxFg}
%% \begin{verbatim}
%%         for mo in cuda_blocks(0, M / 16):
%%             for no in cuda_blocks(0, N / 16):
%%                 # ... CUDA block code [next slide]
%% \end{verbatim}
%% \end{mdframed}
%% \mySub{Scheduling Code}
%% \begin{verbatim}
%% def schedule_gemm(p: Proc ...):
%%     # ...
