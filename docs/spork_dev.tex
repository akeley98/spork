...
One read/write visibility record added for \myKey{each} read/write of a variable
...
\node(reads) [normalnode, below=of vars, xshift=-25mm, minimum width=4cm, text width=4cm] {read visibility records};
\node(writes) [normalnode, below=of vars, xshift=+25mm, minimum width=4cm, text width=4cm] {write visibility record};
...
\draw [arrow] (sigthread) -- (tid);
\draw [arrow] (sigthread.east) -- (signature);
\draw [arrow] (AO) -- (signature);
\end{tikzpicture}
\end{minipage}
\newpage
