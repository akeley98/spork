...
When we interpret \lighttt{Arrive(L1, \textasciitilde 0) >> $Q$}, it's like running
\lighttt{Arrive(L1, 1) >> $Q$} in a loop, with the iteration count being
\begin{itemize}
  \item $Q^1 - Q^2$, if there is no back queue barrier array for the barrier variable that $Q$ is from.
...
Assume here that $Q$ denotes a queue barrier from the front queue barrie r array.
