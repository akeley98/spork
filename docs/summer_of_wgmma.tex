...
The variable name and \lighttt{cuda\_threads} iterators get used for distributed memory deduction (which ignores multicasting).
...
\begin{itemize}
  \item Let $Q$ be the \myKeyA{home queue barrier}, i.e., the queue barrier named by the variable name and iterators deduced above (this is \lighttt{ringbar[m\_cta, n\_cta]} in all above examples).
...
\filbreak
TODO either extend this for \lighttt{N = \textasciitilde 0}, or kill off the \lighttt{N = \textasciitilde 0} feature.
...
\begin{itemize}
  ...
  \filbreak
  \item If there is no \lighttt{ReverseAwait} or \lighttt{ReverseArrive}, we also support \lighttt{Arrive} on $Q$ with $N=1$, followed by an unconditional \lighttt{Await} on $Q$ [arrive-first usage]
...
\mainKey{Static Analysis:} for each \lighttt{[Reverse]Arrive} statement using $Q$, search forward/backwards \textit{syntactically} for the next/previous statement using $Q$ or $\widehat{Q}$ (forwards in the arrive-first case; backwards in the await-first case).
\filbreak
\begin{itemize}
...
However, if both \lighttt{Arrive} and \lighttt{ReverseArrive} are used for a given barrier variable, then the set of CTAs multicast to must be the same for both statements \textbf{(figure \ref{fig:multicast_pairing_fail})}.
TODO unclear if this is actually needed.

\filbreak
\mainSub{TMA Instructions \& mbarrier Pairing}

TMA-to-SMEM instructions, which are the only instructions taking an mbarrier parameter, must appear only between a mbarrier-paired \lighttt{ReverseAwait} and \lighttt{Arrive} statement.
...
\filbreak
\minorKey{Case 1: No Reverse Barrier Array:}
...
\filbreak
\minorKey{Case 2: Reverse Barrier Array:}
Let \lighttt{F\_lag} be the \lighttt{lag} value used for the forward \lighttt{Await} statements and \lighttt{R\_lag} be the \lighttt{lag} values used for \lighttt{ReverseAwait} statements ($N =$ \lighttt{\textasciitilde lag}).
...
\filbreak
\mainKey{Synchronization:} Currently each Exo barrier variable corresponds to either one or two queue barrier arrays, depending on whether \lighttt{ReverseArrive} and \lighttt{ReverseAwait} are used.
TODO improve this.
...
A visibility record \myKeyA{synchronizes-with} $V_1$: set of \lighttt{tl\_sig} when the $V_A$ of that visibility record satisfies $V_A \cap V_1 \ne \varnothing$.
...
\filbreak
Define \lighttt{multicast\_ids(queue\_barrier\_array\_id barriers, int* iterators, multicast\_flags multicast)} to be the set of \lighttt{id: queue\_barrier\_id} where
\begin{itemize}
...
\begin{itemize}
  ...
  \filbreak
  \item If \lighttt{asn.mutate} exists and it synchronizes-with $V_1$, augment it with $V_2^F$.
  \filbreak
  \item For each read visibility record in \lighttt{asn.reads} that synchronizes-with $V_1$, augment it with $V_2^T$.
\end{itemize}
...
\begin{itemize}
  ...
  \item Let $A = \bigcup_{\blacktt{M} \in \blacktt{multicast}} \{ (q, Q^1) \mid q \in \lighttt{multicast\_ids(barriers, iterators, M)} \}$.
  \filbreak
  \item If \lighttt{asn.mutate} exists and it synchronizes-with $V_1$, set $A_p \leftarrow A_p \cup A$.
  \filbreak
  \item For each read visibility record in \lighttt{asn.reads} that synchronizes-with $V_1$, set $A_p \leftarrow A_p \cup A$.
\end{itemize}
