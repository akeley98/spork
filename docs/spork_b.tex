...
In the context of a given \lighttt{Arrive} statement, an iterator (control variable) $y$ is a \myKeyA{multicast iterator} if it appears at some index $n$ of a barrier expression, and another barrier expression has an interval at index $n$.
For example,
\begin{equation*}
...
Each multicast iterator is defined by some \lighttt{cuda\_threads} loop.
