...
\begin{itemize}
  ...
  \item The f \myKeyA{pending await} set (def~\ref{sec:gPendingAwait}), denoted $r.p$, which are tuples of type $\mathbb{B} \times \mathbb{Z}^* \times \mathbb{Z}$, this being a combination of an indexed barrier variable (def~\ref{sec:gMathbbB}) and an arrive-count.
...
The visibility set is encoded as the union of zero or more ``timeline signature intervals'', formatted as the lines \\
...
\begin{align*}
  ...
  & \rho'' = \rho'[z \to n^* \to (r.r, r.m, r.b.a, w')] & \text{(def~\ref{sec:gUpdateNotation})}
\end{align*}
...
Examples
\begin{tabular}{l l}
Arrive statement & Home barrier expression \\
