...
Instruction templates (classes), timelines, atomic instructions, special window e.g., \lighttt{CUtensorMap}.
...
Ampere tensor cores and TMA instructions use collective unit \lighttt{cuda\_warp}.
TMA is specified as a per-thread instruction in PTX, but the implementation of TMA in Exo-GPU expands this to a warp instruction, with \lighttt{elect\_one\_sync} used within the instruction implementation.
This matches the expected usage pattern and underlying hardware reality\footnote{``TMA programming model is Uniform: all active threads in a warp are executing TMA operation 'with same arguments' '', GTC S62192, ``Advanced Performance Optimization in CUDA'', Igor Terentyev} better.
...
\begin{itemize}
  ...
  \end{align*}
\end{itemize}
This procedure takes care of the common case where $U=0$.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\textit{box} = \delta.B_k$  % TODO fix
