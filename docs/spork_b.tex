...
Within the device task, each statement instance (def~\ref{sec:gStmtInstance}) is executed by a set of threads within the cluster; this is a \myKeyA{thread collective} (def~\ref{sec:gThreadCollective}).

A \lighttt{cuda\_threads(0, $c_\text{hi}$, unit=$\tau_u$)} loop may appear anywhere inside a device task.
...
The \lighttt{unit} parameter is a \myKeyA{collective unit} $\tau_u$ (def~\ref{sec:gCollUnit}), from which a \myKeyA{collective type} $\delta$ is unpacked (Section~\ref{sec:CollType}).
...
We need this latter attribute to be able to reason about async instructions; the qualitative timeline of a memory access varies depending on what instruction performs the access (Section~\ref{sec:Instr}).
...
The barrier variable itself is allocated with the syntax ``\texttt{<name>: barrier[$n^*$] @ $\pi_z$}'', where $n^*$ defines the array size of the barrier variable and $\pi_z$ defines the barrier mechanism (def~\ref{sec:gBarrierMechanism}), which is one of \lighttt{CudaClusterSync} (Section~\ref{sec:ClusterSyncUsage}), \lighttt{CudaCommitGroup} (Section~\ref{sec:CommitGroupUsage}), or \lighttt{CudaMbarrier} (Section~\ref{sec:MbarrierUsage}).
The barrier array is subject to distributed memory analysis.

The goal of synchronization checking is to validate \myKeyA{sequential-parallel equivalence}.
The value semantics of an Exo program (which defines the numerical outputs) continue to be defined sequentially; we can, in a sense, view the new Exo-GPU constructs as ``annotations'' on sequential code.
...
\begin{itemize}
  \item \texttt{Read|Write $\to$ Read|Write}\hfill(Section~\ref{sec:MultipleMemoryAccesses})\\
  when the prior memory access is not \emph{out-of-order}, and the prior memory access uses a qualitative timeline that is in the \emph{extended qualitative timeline set} of the subsequent memory access.
  \item \texttt{Read|Mutate $\to$ Fence($\tau_s^\textrm{pre}$, \_)|Arrive($\tau_s^\textrm{pre}$) >}\texttt{> \_}\hfill(Section~\ref{sec:AccessBeforeSync})\\
  when the qualitative timeline of the memory access is in the \textit{full timeline set} of $\tau_s^\textrm{pre}$.
  \item \texttt{Fence(\_, $\tau_s^\textrm{post}$)|Await(\_, $\tau_s^\textrm{post}$, \_) $\to$ Read}\hfill(Section~\ref{sec:AccessAfterSync})\\
  when the qualitative timeline of the read is in the \textit{full timeline set} of $\tau_s^\textrm{post}$.
  \item \texttt{Fence(\_, $\tau_s^\textrm{post}$)|Await(\_, $\tau_s^\textrm{post}$, \_) $\to$ Write}\hfill(Section~\ref{sec:AccessAfterSync})\\
  when the qualitative timeline of the read is in the \textit{temporal timeline set} of $\tau_s^\textrm{post}$.
  \item \texttt{Fence(\_, $\tau_s^\textrm{post}$)|Await(\_, $\tau_s^\textrm{post}$, \_) $\to$ Fence($\tau_s^\textrm{pre}$, \_)|Arrive($\tau_s^\textrm{pre}$) >}\texttt{> \_}\hfill(Section~\ref{sec:Transitivity})\\
  when there is a qualitative timeline in common to the \textit{full timeline sets} of $\tau_s^\textrm{pre}$ and $\tau_s^\textrm{post}$.
\end{itemize}

Finally, we have special handling for atomics, there are dependency edges from instances of \lighttt{Arrive(\_) >}\lighttt{> $z$} and \lighttt{Await($z$, \_, \_)} defined by arrive/await pairing, and dependency edges from reads/writes to \lighttt{Await} instances directly for instructions that take a barrier directly (currently, only TMA-to-SMEM instructions, Section~\ref{sec:Tma}).
...
\end{tikzpicture}
\caption{Example of a thread collective being described by the collective type $\delta$ with $\delta.B = (1, 2, 1, 128)$ and $\delta.D = (4, 2, 3, 128)$, which is a collective type in aligned form (def~\ref{sec:gAlignedForm}).
The domain implies an organization of the threads in the cluster into a 4D $4 \times 2 \times 3 \times 128$ space (which is tough to illustrate).
...
\subsection{Collective Dimension Descriptor}
\label{sec:CollDimDescriptor}
...
\subsection{Collective Tiling Derived State}
\label{sec:CollTilingDerivedState}
...
\begin{itemize}
  ...
  \item its tiled dimension index is that of a subdivided dimension
  \item its thread pitch is not 0
\end{itemize}
with ``tiled dimension index'' and ``thread pitch'' as defined by $\omega$ (Section~\ref{sec:CollTilingDerivedState}).
...
\begin{itemize}
  ...
  \item its thread pitch is not 0
\end{itemize}
with ``thread pitch'' as defined by $\omega$ (Section~\ref{sec:CollTilingDerivedState}).
...
\begin{itemize}
  ...
  \item the atomic qualitative timeline, whis is non-empty if the access is atomic, Section~\ref{sec:AtomicInstr}
...
\subsection{Instruction Template (Classes)}
\label{sec:InstrTemplate}
...
\subsection{Instruction Collective Unit}
\label{sec:InstrCollUnit}
...
\begin{equation*}
    \texttt{AllocableMemWin} \to \mathsf{InstrTL} \to \mathsf{QualTL} \times \mathcal{P}(\mathsf{QualTL})\codecomment{~~(initial qual-tl, extended qual-tl set)}
\end{equation*}
...
\subsection{Out-of-order Access}
\label{sec:InstrOOO}
...
\begin{itemize}
  ...
  \begin{align*}
    ...
    & \text{where } \delta_i \text{ is unpacked from } \tau_u^i \text{(Section~\ref{sec:gCollUnit})}
  \end{align*}
...
\begin{itemize}
  ...
  \item $e_i = 1$ if $e_i$ is a point expression
\end{itemize}
...
\newpage
\subsection{Instruction Codegen}
\label{sec:InstrCodegen}
...
\subsection{\texttt{cuda\_tasks} Loop}
\label{sec:gCudaTasksLoop}
...
\subsection{\texttt{cuda\_threads} Loop}
\label{sec:gCudaThreadsLoop}
...
\subsection{\texttt{CudaWarps} Block}
\label{sec:gCudaWarpsBlock}
...
% >E
\subsection{Extended Qualitative Timelines (Set)}
\label{sec:gExtQualTL}
...
\subsection{Initial Qualitative Timeline}
\label{sec:gInitialQualTL}

Qualitative timeline (def~\ref{sec:gQualTL}) associated with each read/mutate memory access.
This varies depending on the memory type accessed and the instruction used (Section~\ref{sec:InstrTL}).
In abstract machine semantics, this controls \lighttt{VisRecord} creation (Section~\ref{sec:VisRecordCreation}).

\subsection{Instruction Template}
\label{sec:gInstrTemplate}
...
\subsection{Instruction Template Parameter}
\label{sec:gInstrTemplateParameter}
...
\subsection{Intact Dimension}
\label{sec:gIntactDimension}
...
Distributed memory analysis physically maps each shard of a variable annotated with this \lighttt{Memory} type onto a thread collective (def~\ref{sec:gThreadCollective}) described by the collective type (Section~\ref{sec:CollTypeThreadCollective}) unpacked from this collective unit.
...
\subsection{Sequential (\texttt{seq}) Loop}
\label{sec:gSeqLoop}
...
\subsection{Sequential-parallel Equivalence}
\label{sec:gSeqParEquivalence}
...
\subsection{Subdivided Dimension}
\label{sec:gSubdividedDimension}

See ``Distributed Memory'' (Section~\ref{sec:DistributedMemory}).

\subsection{Sync Check}
\label{sec:gSyncCheck}
...
Set of threads within a single CUDA cluster (def~\ref{sec:gCluster}) assigned to execute one statement instance (def~\ref{sec:gStmtInstance}) or to allocate one shard of a variable (Section~\ref{sec:DistributedMemory}).
...
\subsection{wgmma Fence}
\label{sec:gWgmmaFence}
...
\subsection{Parallelism Overview}
...
Calls to the instruction must appear at $\delta$-scope (Section~\ref{sec:CollTilingCollTypeMatching}).
...
\subsection{Sequential-Parallel Equivalence Overview}
...
The user schedules the procedure by issuing a series of rewrite operations $R_1 ... R_{N-1}$ to create transformed procedures $p_2 ... p_N$.
Key to checking functional equivalence between the simple initial procedure ($p_1$) and the generated CUDA program is our notion of \myKeyA{sequential-parallel equivalence}.
...
Checking sequential-parallel equivalence is the purpose of sync check (Section~\ref{sec:SyncSemantics}), which we currently only implement when given a concrete problem size.
...
In this case, we still have a guarantee that the final program, interpreted under sequential semantics, will be functionally equivalent to the generated CUDA program (at least for the set of problem sizes given to sync check).

Key to this checking is that each statement in CUDA scope (def~\ref{sec:gCudaScope}) is annotated with a \myKeyA{collective tiling} (Section~\ref{sec:CollTiling}), which statically encodes which threads within the CUDA cluster (def~\ref{sec:gCluster}) are used for what work.
...
This design would not be appropriate for Exo-GPU as it would undermine sequential-parallel equivalence. % TODO illustrate problem in figure?
In Exo-GPU, the size of each allocated tensor is whatever its \emph{logical} size should be.
Distributed memory is deduced from the indexing pattern of an array, and attempts to map shards of the array into thread collectives described by the collective type specified by the memory type of the tensor.
...
% Parallelism (thread collective)
% Instruction selection
%   thread collective
%   qual-tl, atomicity, convergent, out-of-order
% Barriers
%   mem access before
%   Arrive/Await mem access before & mem access after when "paired"
% Seq/par equivalence
% Distributed memory
